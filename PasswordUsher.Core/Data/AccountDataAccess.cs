			...
			try {
				using (var connection = SqlHelper.GetConnection ()) {			
					connection.Execute (@"insert into Accounts values(null, @name, @password, @providerId)",
					                    new {name = entity.Name, password = Encryptor.Encrypt<RijndaelManaged>(entity.Password, ConfigHelper.EncyptionPassword, ConfigHelper.EncyptionKey)  , providerId = entity.ProviderId });							
				}	
				return true;
			...
			try {
				using (var connection = SqlHelper.GetConnection ()) {			
					connection.Execute (@"update Accounts set Name = @name, Password  = @password, ProviderId = @providerId where Id = @Id;",
					                    new {name = entity.Name, password = Encryptor.Encrypt<RijndaelManaged>(entity.Password, ConfigHelper.EncyptionPassword, ConfigHelper.EncyptionKey), providerId = entity.ProviderId, id = entity.Id });
				}	
				return true;
